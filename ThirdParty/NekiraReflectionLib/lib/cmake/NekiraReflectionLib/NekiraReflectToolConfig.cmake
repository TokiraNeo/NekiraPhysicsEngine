...
# ========================================
# 搜索NekiraReflectTool.exe
# ========================================
find_program(NekiraReflectTool_EXE
    NAMES NekiraReflectTool
    HINTS "${PACKAGE_PREFIX_DIR}/bin"
)

# 如果未找到NekiraReflectTool，则报错
if(NOT NekiraReflectTool_EXE)
    message(FATAL_ERROR "NekiraReflectTool.exe not found.")
endif()

# 设置NekiraReflectTool的路径
set(NekiraReflectTool_Path "${NekiraReflectTool_EXE}")

message(NOTICE "NekiraReflectTool_Path: ${NekiraReflectTool_Path}")
...
function(NekiraReflection_AddModule)
    ...
    set(multiValueArgs HEADERS INCLUDE_PATHS DEFINITIONS OPTIONS)

    # 解析参数(以NREFL_开头)(必须传递至少4个参数，所以这里的${ARGN}是占位符)
    cmake_parse_arguments(NREFL "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
    ...
    # 添加include路径
    # 确保添加NekiraReflectDynamic库的include路径，这样NekiraReflectTool才能识别其中的有关头文件
    list(APPEND REFLECT_TOOL_ARGS "-I${PACKAGE_PREFIX_DIR}/include")
    ...
    # 添加额外的可选参数
    if(NREFL_OPTIONS)
        foreach(OPTION ${NREFL_OPTIONS})
            list(APPEND REFLECT_TOOL_ARGS "${OPTION}")
        endforeach()
    endif()
    ...
    if(GENERATED_HEADER_LIST OR GENERATED_SOURCE_LIST)
        ...
        # 将生成的源文件，头文件添加到原目标(这里没有必要，因为生成的反射代码已经是自定义目标，而原目标依赖自定义目标)
